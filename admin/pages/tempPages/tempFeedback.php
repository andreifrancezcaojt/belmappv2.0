...
<table class="table table-striped table-hover" id="myTable">
    ...
    $q = 'SELECT id, feedback_url FROM feedback_qr';

    if (isset($_GET['srch'])) {
        if ($_GET['srch'] != '') {
            $q .= ' WHERE (feedback_url LIKE \'%' . $_GET['srch'] . '%\')';    
        }
    }
    ...
    if(mysqli_num_rows($rs) > 0){
        while ($rw = mysqli_fetch_array($rs)) {
            echo '<tr>
                    <td>' . $rw['id'] . '</td>
                    <td>' . $rw['feedback_url'] . '</td>
                    <td>
                        <div style="display:flex; gap: 3px;">
                            <a class="btn btn-warning btn-sm" href="javascript:void(0);" onclick="TINY.box.show({url:\'admin/pages/edit_qr.php?id='.$rw['id'].'\',width:400,height:450})">
